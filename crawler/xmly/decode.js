...
function p(arr, offset, key) {
  const len = Math.min(arr.length - offset, key.length);
  for (let i = 0; i < len; i++) {
    arr[offset + i] ^= key[i];
  }
}
...
function getSoundCryptLink(e) {
  ...
  let u = o;
  let c = a;
  if (!["www2", "mweb2"].includes(i)) {
    u = r;
    c = n;
  }
  try {
    const base64Decoded = Buffer.from(t.replace(/_/g, "/").replace(/-/g, "+"), 'base64').toString('binary');
    let e = base64Decoded;
    if (e === null || e.length < 16) return t;
    let decrypted = new Uint8Array(e.length - 16);
    for (let idx = 0; idx < e.length - 16; idx++) {
      decrypted[idx] = e.charCodeAt(idx);
    }
    let key = new Uint8Array(16);
    for (let idx = 0; idx < 16; idx++) {
      key[idx] = e.charCodeAt(e.length - 16 + idx);
    }
    for (let idx = 0; idx < decrypted.length; idx++) {
      decrypted[idx] = u[decrypted[idx]];
    }
    for (let idx = 0; idx < decrypted.length; idx += 16) {
      p(decrypted, idx, key);
    }
    for (let idx = 0; idx < decrypted.length; idx += 32) {
      p(decrypted, idx, c);
    }
    return utf8Encode(decrypted);
  } catch (err) {
    console.warn(err, "secret failed");
    return "";
  }
}
...
if (process.argv.length > 2) {
  ...
  console.log(getSoundCryptLink({ link: encrypted }));
} 